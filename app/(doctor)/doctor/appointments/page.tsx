...
export default function DoctorAppointmentsPage() {
  ...
  // Filter for TODAY
  const getTodayFiltered = (appointments: Appointment[]) => {
    if (filter !== "TODAY") return appointments;
    const today = new Date().toISOString().split('T')[0];
    return appointments.filter(a => a.date === today);
  };

  const visible = getTodayFiltered(appointments).filter(a => {
    const matchesFilter = filter === "All" || filter === "TODAY" || a.status === filter;
    const matchesSearch =
  ...
  // Get doctor name for TopBar
  const doctorName = "Dr. Julianne Case";
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-10 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <input
            ...
            className="bg-white border border-[#F1F5F9] rounded-lg px-4 py-2 text-sm text-[#6B7280] outline-none focus:border-[#00685C] w-64"
          />
        </div>
        ...
        <div className="bg-white border border-[#F1F5F9] rounded-xl overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            ...
            <thead className="bg-[#F8FAFC] border-b border-[#F1F5F9]">
              <tr>
                {["ID", "PATIENT", "PROCEDURE", "DATE & TIME", "STATUS"].map(h => (
              ...
              {loading ? (
                ...
                visible.map((appt, i) => {
                  ...
                  return (
                    <tr
                      key={appt.rawId}
                      onClick={() => canManage && openPanel(appt)}
                      className={`${i > 0 ? "border-t border-[#F8FAFC]" : ""} ${
                        canManage ? "cursor-pointer hover:bg-[#F8FAFC]" : "cursor-default"
                      } transition-colors ${selectedAppt?.rawId === appt.rawId ? "bg-[#F0FDFA]" : ""}`}
                    >
                      ...
                      <td className="px-6 py-4 text-sm text-[#3D4946] max-w-[200px] truncate">
                        {appt.observation?.split('.')[0] || "No procedure specified"}
                       </td>
                      <td className="px-6 py-4">
      ...
      {showDoctorPanel && selectedAppt && (
        <div className="fixed top-0 right-0 bottom-0 w-80 bg-white shadow-2xl z-[60] flex flex-col">
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-5 border-b border-[#F1F5F9]">
            <div>
