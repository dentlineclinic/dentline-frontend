  ...
  return (
    <div className="flex items-center justify-between">
      <h2 className="text-xl font-bold text-[#0B1C30]">
        {calendarData?.monthName || "Loading..."} {calendarData?.year || ""}
      </h2>
      <div className="flex items-center gap-2">
        <button
          onClick={() => onNavigate(calendarData?.previousYear || new Date().getFullYear(), calendarData?.previousMonth || 1)}
        ...
        <button
          onClick={() => onNavigate(calendarData?.nextYear || new Date().getFullYear(), calendarData?.nextMonth || 1)}
          className="w-9 h-9 rounded-lg border border-[#E2E8F0] bg-white flex items-center justify-center text-[#3D4946] hover:bg-[#F0FDFA] hover:border-[#00685C] transition-colors"
